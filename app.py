...
class SimpleMySQL:
    def __init__(self, app):
        ...
        self.connection = self._create_proxy()
        self.connect()
    ...
    def _create_proxy(self):
        class ConnectionProxy:
            ...
            def cursor(self):
                return self._owner.get_connection().cursor()

            def commit(self):
                return self._owner.get_connection().commit()

            def close(self):
                return self._owner.close()

            def __getattr__(self, name):
                return getattr(self._owner.get_connection(), name)
    ...
    def connect(self):
        try:
            self._connection = pymysql.connect(
                host=self.app.config.get('MYSQL_HOST', os.getenv('MYSQL_HOST', 'localhost')),
                user=self.app.config.get('MYSQL_USER', os.getenv('MYSQL_USER', 'root')),
                password=self.app.config.get('MYSQL_PASSWORD', os.getenv('MYSQL_PASSWORD', '')),
                database=self.app.config.get('MYSQL_DB', os.getenv('MYSQL_DB', 'grillista_admin')),
            ...
            self._connection = None
            print(f"MySQL connection unavailable: {e}")
...
app.config['MYSQL_DB'] = os.getenv('MYSQL_DB', 'grillista_admin')
